...
export const updateRaidReport = debounce(async (message: string, channel: string) => {
  ...
  if (!thread?.isThread()) {
    throw new Error(`Failed to update raid report: channel ${words[1]} is not a thread`);
  }
