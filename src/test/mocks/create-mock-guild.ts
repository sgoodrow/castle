...
import { MockTextChannel, createMockTextChannel } from "./create-mock-text-channel";
import { MockThreadChannel, createMockThreadChannel } from "./create-mock-thread-channel";
...
export function createMockGuild({
  ...
  textChannels.forEach(({ id: channelId, channel }) => {
    channelMap.set(channelId, channel || createMockTextChannel({ id: channelId }));
  });

  threadChannels.forEach(({ id: channelId, channel }) => {
    channelMap.set(channelId, channel || createMockThreadChannel({ id: channelId }));
  });

  const fetchMock =
    createTypedMock<(id: string) => Promise<TextChannel | PublicThreadChannel | null>>();

  // Set up the mock implementation to return channels from the map
  fetchMock.mockImplementation((channelId: string) => {
    const channel = channelMap.get(channelId);
    return Promise.resolve(channel as unknown as TextChannel | PublicThreadChannel | null);
  });
